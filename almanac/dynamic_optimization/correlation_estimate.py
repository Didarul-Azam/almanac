...
class correlationEstimate(genericMatrixEstimate):
    ...
    def shrink(self, prior_corr: "correlationEstimate",
        ...
        shrunk_corr = (
            shrinkage_corr * prior_corr_values +
...
def create_boring_corr_matrix(
    ...
    corr_matrix_values = boring_corr_matrix_values(
        size, offdiag=offdiag, diag=diag)

    boring_corr_matrix = correlationEstimate(
        corr_matrix_values, columns=columns)
...
def calculate_weekly_percentage_returns(adjusted_price: pd.Series,
    ...
    weekly_adj_prices = adjusted_price.reindex(
        weekly_common_index, method="ffill")
    weekly_current_prices = current_price.reindex(
        weekly_common_index, method="ffill")
...
def get_correlation_estimate_at_date(relevant_date: datetime,
    ...
    corr_matrix_values = (
        exp_correlations[exp_correlations.index.get_level_values(
            0) < relevant_date]
        .tail(size_of_matrix)
        .values
    )
        ...
        [
            (key_name,
             get_row_of_series_before_date(
                 ts_series, relevant_date=relevant_date)
             )
...
def calculate_covariance_given_correlation_and_stdev(
    ...
    all_assets = set(list(correlation_estimate.columns) +
                     stdev_estimate.list_of_keys())
    ...
    aligned_stdev_list = stdev_estimate.list_in_key_order(
        list_of_assets_with_data)
    ...
    covariance = covariance_assets_with_data.add_assets_with_nan_values(
        assets_without_data)
